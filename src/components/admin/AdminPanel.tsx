...
import { RegionCoverManager } from "./RegionCoverManager";
import "./admin.css";
...
type Tab = "dashboard"|"properties"|"regions"|"leads"|"channels"|"profile";
...
export function AdminPanel(){
  ...
  return <section className="admin-shell"><div className="admin-wrap">
    ...
    <nav className="admin-tabs" aria-label="Seções do painel">{([['dashboard','Visão geral'],['properties','Imóveis'],['regions','Regiões'],['leads','Leads'],['channels','Portais'],['profile','Perfil']] as [Tab,string][]).map(([key,label])=><button key={key} type="button" aria-current={tab===key?"page":undefined} onClick={()=>setTab(key)} className={`admin-tab ${tab===key?"is-active":""}`}>{label}</button>)}</nav>
    {message&&<div className="admin-message">{message}</div>}
    {tab==="regions"&&<RegionCoverManager />}
